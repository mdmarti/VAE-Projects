...
def run_stochastic_lorenz_experiment():
   ...
   #plot_sde(xs)
	...
	xs,(mu,d),(eta,lam) = generate_stochastic_lorenz(1024,dt=dt,T = 1,coeffs=[10,28,8/3,.15,.15,.15],zscore=True)
	xsTest,_,_ = generate_stochastic_lorenz(1024,dt=dt,T = 1,coeffs=[10,28,8/3, .15,.15,.15],zscore=True)
	print('done!')
	#plotSamples3d(xs,xs)
	...
	modelMoments = nonlinearLatentSDE(dim=3,save_dir=f'/home/miles/lorenz_normal_lr_{lr}_3hidden_hiddensize200_plateauLR_z',plotDists=True,\
							diag=False,true1=mu,true2=d,n_hidden=3,hidden_size=200)
	modelNatParms = nonlinearLatentSDENatParams(dim=3,save_dir=f'/home/miles/lorenz_natparms_lr_{lr}_3hidden_hiddensize200_plateauLR_z',plotDists=True,\
	...
	modelMoments = train(modelMoments,dls,nEpochs=10000,save_freq=500,test_freq=100,lr=lr)
	modelNatParms = train(modelNatParms,dls,nEpochs=10000,opt='adam',save_freq=500,test_freq=100,lr=lr)
...
def run_2d_swirly_boy(zscore=False):
	...
	#plotSamples2d([(d + muTr)*sdTr for d in x],[(s + muTr)*sdTr for s in samples1])
	#plotSamples2d([(d + muTr)*sdTr for d in x],[(s + muTr)*sdTr for s in samples2])
