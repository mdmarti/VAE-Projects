...
class predictor(nn.Module):
	...
	def __init__(self,z_dim=128,h_dim=64):
		...
		self.predictor_fc = nn.Sequential(nn.Linear(self.z_dim,self.h_dim),
									...
									nn.ReLU())
		self.A = nn.Linear(self.z_dim,self.z_dim**2)
		self.B = nn.Linear(self.z_dim,self.z_dim**2)

	def forward(self, z):

		p = self.predictor_fc(z)
		A = self.A(p)
		A = A.view(-1,self.z_dim)
		B = self.B(p).exp()
		B = torch.tril(B.view(-1,self.z_dim,self.z_dim))

		return A, B
	
	def sample(self,z,dt):
		
		A,B = self.forward(z)
		zNext = z + A*dt + np.sqrt(dt) * B @ np.random.randn(z.shape)
...
class simsiam(nn.Module):
	...
	def sde_forward(self,z):
		
		A,B = self.predictor(z)
		return A,B

	def compute_loss(self,zt,zdt,A,B,dt):

		z_loss = zdt.detach()
		
		cov = dt * torch.matmul(B, B.transpose(-2,-1))
		ld = torch.logdet(cov)
		quadratic = (z_loss - zt - A*dt).transpose(-2,-1) @ torch.cholesky_inverse(np.sqrt(dt) * B) @ (z_loss - zt - A*dt)
		return 1/2 *(ld + quadratic)
	...
	def train_epoch(self, loader):
		...
		for ii, batch in enumerate(loader):
			...
			z1 = self.forward(x1)
			z2 = self.forward(x2)
			A,B = self.sde_forward(z1)
			L = self.compute_loss(z1,z2,A,B,dt)
